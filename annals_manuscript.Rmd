...
# gets the meta data 
  ...
  # This country versus nationality
  p3<-   ggviolin(pseq.meta, x = "nationality", y = "Shannon",
                  add = "boxplot", fill = "nationality", palette = "paired") + theme(axis.text.x = element_text(angle = 90)) + theme(legend.position="none") + xlab("")

  # the number in the palettes must equal the number of groups in bmi.pairs
  
  p4 <<- ggviolin(data=subset(pseq.meta, !is.na(mnd)), x = "mnd", y = "Shannon", add = "boxplot", fill = "mnd", na.rm = TRUE, palette = c("yellow", "orange", "red")) 
  
  p4 <<- p4 + stat_compare_means(comparisons = mnd.pairs) + xlab("MND DALYs") + theme(legend.position="none")
...
p.1 <<-plot_grid(title, p1, p3, p2, p4, align = "v" , ncol = 2, labels = c('','A', 'B', 'C', 'D'), rel_widths = 1, rel_heights = 2)
print(p.1)
